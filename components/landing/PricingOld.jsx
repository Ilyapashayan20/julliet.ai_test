...
function Plan({ name, price, description, href, features, featured = false }) {
  return (
    ...
    >
      <h3
        className={clsx('mt-5 text-lg font-display', featured && 'text-white')}
      >
        {name}
      </h3>
      <p
...
export function Pricing() {
  ...
  const moveTooltip = (e) => {
    tooltipRef.current.style.marginLeft = `${(e.target.value - 50) * 1.48}%`;
    ...
    <section id="pricing" aria-label="Pricing" className="py-20 sm:py-32">
      <Container>
        ...
        <div className="relative flex flex-col items-center justify-center pt-1 mt-8 overflow-hidden">
          <div
            className="flex flex-col text-center text-md h-[70px] lg:h-[65px] rounded-md bg-slate-200"
            ref={tooltipRef}
          >
